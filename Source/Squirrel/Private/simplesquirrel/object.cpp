...
namespace ssq {
    HSQUIRRELVM Object::caller_vm=nullptr; 
...
#ifdef SQUNICODE
    Object Object::find(const FString &name) const {
      if (vm == nullptr) throw RuntimeException("VM is not initialised");
    ...
    bool Object::contains(const FString &name) const {
      if (vm == nullptr)
        return false;
