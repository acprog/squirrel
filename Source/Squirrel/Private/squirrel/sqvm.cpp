    ...
    if((tcs = tc.size())) {
        for(SQInteger i = 0; i < nargs && i < tcs; i++) {
            if((tc._vals[i] != -1) && !(sq_type(_stack._vals[newbase+i]) & tc._vals[i])) {
                // AC: разрешить передавать в параметры ф-ии int как float и float как int
                if (tc._vals[i] & OT_INTEGER && sq_type(_stack._vals[newbase + i]) == OT_FLOAT) {
                  _stack._vals[newbase + i]._type = OT_INTEGER;
                  _stack._vals[newbase + i]._unVal.nInteger = _stack._vals[newbase + i]._unVal.fFloat;
                }
                else if (tc._vals[i] & OT_FLOAT && sq_type(_stack._vals[newbase + i]) == OT_INTEGER) {
                  _stack._vals[newbase + i]._type = OT_FLOAT;
                  _stack._vals[newbase + i]._unVal.fFloat = _stack._vals[newbase + i]._unVal.nInteger;
                } else {
